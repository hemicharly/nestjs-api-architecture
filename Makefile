...
# configured in the docker-compose.cli.yml file.
install:
	yarn install &&\
...
# Run coverage
test-coverage:
	docker compose -f docker-compose.cli.yml run --rm yarn test:coverage

# Run to update readme coverage
readme-coverage:
	docker compose -f docker-compose.cli.yml run --rm yarn readme:coverage

# Updates all project libraries to their latest versions using Yarn,
# running in a container defined in the docker-compose.cli.yml file.
upgrade-lib:
	docker compose -f docker-compose.cli.yml run --rm yarn upgrade --latest

# Runs the seed script (to populate the database or initialize data) using Yarn,
# in a container defined in the docker-compose.cli.yml file.
seed:
	docker compose -f docker-compose.cli.yml run --rm yarn seed

# Generates the project's index (e.g., files or reference code) using Yarn,
# in a container defined in the docker-compose.cli.yml file.
generate-index:
	docker compose -f docker-compose.cli.yml run --rm yarn generate-index && \
	git add .

# Generates the test file
generate-test-file:
	docker compose -f docker-compose.cli.yml run --rm yarn generate-test-file
...
# and then builds the project using Yarn in a container defined in the docker-compose.cli.yml file.
lint-format:
