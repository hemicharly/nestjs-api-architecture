...
export class OrderEndUsecaseImpl implements OrderEndUsecaseInterface {
  ...
  public async execute(orderEndCoreEntity: OrderEndCoreEntity): Promise<void> {
    OrderCoordsCoreEntity.validateCoordinates(
      orderEndCoreEntity.recordedLatitude,
      orderEndCoreEntity.recordedLongitude
    );

    const entityCore = await this.repositoryProvider.findByIdAndEmployeeId(
      orderEndCoreEntity.id,
      orderEndCoreEntity.employeeId
    );

    OrderEndCoreEntity.validateOrderEnd(entityCore);

    ValidateDatesCoreEntity.validateBetweenDates(
      entityCore.startDatetime,
      orderEndCoreEntity.endDatetime
    );
    ...
    const notificationOrderCoreEntity = NotificationOrderCoreEntity.fromOrderEndCoreEntity(
      orderEndCoreEntity,
      entityCore,
